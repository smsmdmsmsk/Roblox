...
local Tabs = {
	Main = Window:AddTab("菜单"),
    Exploits = Window:AddTab("绕过"),
    Visuals = Window:AddTab("绘制"),
    Floor = Window:AddTab("楼层"),
	["UI Settings"] = Window:AddTab("UI Settings"),
}

local _YexHub_custom_captions = Instance.new("ScreenGui") do
    local Frame = Instance.new("Frame", _YexHub_custom_captions)
    local TextLabel = Instance.new("TextLabel", Frame)
    local UITextSizeConstraint = Instance.new("UITextSizeConstraint", TextLabel)

    _YexHub_custom_captions.Parent = ReplicatedStorage
    _YexHub_custom_captions.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    ...
    function Script.Functions.Captions(caption: string)
        if _YexHub_custom_captions.Parent == ReplicatedStorage then _YexHub_custom_captions.Parent = gethui() or game:GetService("CoreGui") or playerGui end
...
getgenv()._internal_unload_YexHub = function()
    Library:Unload()
end
...
function Script.Functions.DoorESP(room)
    ...
    if door then
        ...
        local doorState = if opened then " [已开]" elseif locked then " [锁]" else ""
        ...
        Script.Connections[room.Name .. "Opened"] = door:GetAttributeChangedSignal("Opened"):Connect(function()
            ...
            if doorEsp then doorEsp.SetText(string.format("门 %s [已开]", doorNumber)) end
        end)
    end
end 
...
function Script.Functions.ChestESP(chest)
    ...
    Script.Functions.ESP({
        Type = "Chest",
        Object = chest,
        Text = if locked then "宝箱 [锁]" else "宝箱",
        ...
        if character then
            Script.Connections["AnticheatBypassTheMines"] = character:GetAttributeChangedSignal("Climbing"):Connect(function()                                
                if Toggles.TheMinesAnticheatBypass.Value and character:GetAttribute("Climbing") then
                    ...
                    if workspace:FindFirstChild("_internal_YexHub_acbypassprogress") then workspace:FindFirstChild("_internal_YexHub_acbypassprogress"):Destroy() end
                end
            end)
        end
...
local AntiEntityGroupBox = Tabs.Exploits:AddLeftGroupbox("防实体") do
    ...
    AntiEntityGroupBox:AddToggle("AntiEyes", {
        Text = "防" .. (isBackdoor and "眼睛人" or "眼睛"),
        Default = false
    })
...
local ESPTabBox = Tabs.Visuals:AddLeftTabbox() do
    local ESPTab = ESPTabBox:AddTab("透视功能") do
...
local SelfGroupBox = Tabs.Visuals:AddRightGroupbox("玩家") do
...
task.spawn(function()
    if isHotel then
        local Hotel_AntiEntityGroupBox = Tabs.Floor:AddLeftGroupbox("防实体") do
    ...
    elseif isMines then
        ...
        local Mines_AutomationGroupBox = Tabs.Floor:AddRightGroupbox("Automation") do
            Mines_AutomationGroupBox:AddButton({
                Text = "一键200门闸门",
                Func = function()
                    if latestRoom.Value < 99 then Script.Functions.Alert("你还没到200门..") end
        ...
        Toggles.TheMinesAnticheatBypass:OnChanged(function(value)
            if value then
                local progressPart = Instance.new("Part", workspace) do
                    ...
                    progressPart.Name = "_internal_YexHub_acbypassprogress"
    ...
    elseif isBackdoor then
        ...
        local Backdoors_VisualGroupBox = Tabs.Floor:AddRightGroupbox("显示") do
        ...
        Toggles.AntiHasteJumpscare:OnChanged(function(value)
            ...
            local internal_temp_YexHub = clientRemote:FindFirstChild("_YexHub")
            
            if not internal_temp_YexHub then internal_temp_YexHub = Instance.new("Folder", clientRemote); internal_temp_YexHub.Name = "_YexHub" end

            if value then
                for i,v in pairs(clientRemote.Haste:GetChildren()) do
                    if v:IsA("RemoteEvent") then continue end

                    v.Parent = internal_temp_YexHub
                end
            else
                for i,v in pairs(internal_temp_YexHub:GetChildren()) do
    ...
    elseif isRooms then
        ...
        Toggles.AntiA90:OnChanged(function(value)
            if Toggles.AutoRooms.Value and not value then
                Script.Functions.Alert("防A90需要开启自动Rooms才能正常工作", 5)
        ...
        local _internal_YexHub_pathfinding_nodes = Instance.new("Folder", workspace) do
            _internal_YexHub_pathfinding_nodes.Name = "_internal_YexHub_pathfinding_nodes"
        end

        Toggles.ShowAutoRoomsPathNodes:OnChanged(function(value)
            for _, node in pairs(_internal_YexHub_pathfinding_nodes:GetChildren()) do
            ...
            if value then
                ...
                local function doAutoRooms()
                    ...
                    if path.Status == Enum.PathStatus.Success then
                        ...
                        _internal_YexHub_pathfinding_nodes:ClearAllChildren()

                        for i, waypoint in pairs(waypoints) do
                            local node = Instance.new("Part", _internal_YexHub_pathfinding_nodes) do
                        ...
                        for i, waypoint in pairs(waypoints) do
                            ...
                            if not Toggles.AutoRooms.Value then
                                ...
                                if _internal_YexHub_pathfinding_nodes:FindFirstChild("_internal_node_" .. i) then
                                    _internal_YexHub_pathfinding_nodes:FindFirstChild("_internal_node_" .. i):Destroy()
                                end
                            end
...
Toggles.FakeRevive:OnChanged(function(value)
    if value and alive and character and not fakeReviveEnabled then
        ...
        Script.Functions.Alert("请想办法死亡或等待大约20秒\n假复苏工作。", 20)
        ...
        if alive and not Toggles.FakeRevive.Value then
            ...
            Script.Functions.Alert("假复活已被禁用，无法杀死球员.")
    ...
    Library:GiveSignal(clientRemote.Haste.Remote.OnClientEvent:Connect(function(value)
        if not value and Toggles.NotifyEntity.Value then
            haste_incoming_progress = Instance.new("Part", workspace) do
                ...
                haste_incoming_progress.Name = "_internal_YexHub_haste"
...
Library:OnUnload(function()
    ...
    if isBackdoor then
        local clientRemote = floorReplicated.ClientRemote
        local internal_temp_YexHub = clientRemote:FindFirstChild("XSKil")

        if internal_temp_YexHub and #internal_temp_YexHub:GetChildren() ~= 0 then
            for i,v in pairs(internal_temp_YexHub:GetChildren()) do
                v.Parent = clientRemote.Haste
            end
        end

        internal_temp_YexHub:Destroy()
    end

    if isRooms then
        if workspace:FindFirstChild("_internal_YexHub_pathfinding_nodes") then
            workspace:FindFirstChild("_internal_YexHub_pathfinding_nodes"):Destroy()
        end
    end

    if _YexHub_custom_captions then
        _YexHub_custom_captions:Destroy()
    end
...
MenuGroup:AddLabel("菜单绑定"):AddKeyPicker("MenuKeybind", { Default = "RightShift", NoUI = true, Text = "Menu keybind" })
MenuGroup:AddButton("关闭", function() Library:Unload() end)
CreditsGroup:AddLabel("deividcomsono - 脚本开发")
CreditsGroup:AddLabel("upio - 老大")
CreditsGroup:AddLabel("汉化作者：老大")
CreditsGroup:AddLabel("汉化作者:老大")
CreditsGroup:AddLabel("mstudio45 - 假复活 & firepp")

CreditsGroup:AddDivider()
CreditsGroup:AddLabel("此脚本作者：小玄奘")
CreditsGroup:AddLabel("副脚本作者：YEX")
CreditsGroup:AddLabel("群聊：917126702")
...
ThemeManager:SetFolder("24DOORS")
SaveManager:SetFolder("汉化doors")
